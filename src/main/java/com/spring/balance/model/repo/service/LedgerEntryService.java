...
public class LedgerEntryService {
	...
	public PageResult<LedgerEntryListItem> search(BalanceType type, LedgerEntrySearch search, String username, int page,
			int size) {
	...
	public LedgerEntryForm findForEdit(String id) {
		...
		return EntityOperation.safeCall(ledgerEntryRepo.findById(entryPK).map(LedgerEntryForm::from), "ledger entry",
				"entry id", id);
	}
	...
	private String insert(Member member, LedgerEntryForm entryForm) {
		...
		var amount = entryForm.getItems().stream().filter(a -> !a.isDeleted())
	...
	private String update(Member member, LedgerEntryForm entryForm) {
		...
		// Insert all items
		var newItems = entryForm.getItems().stream().filter(a -> !a.isDeleted()).toList();
		...
		var entries = ledgerEntryRepo.findRemainingEntries(member.getId(), entry.getId().getIssueDate(),
