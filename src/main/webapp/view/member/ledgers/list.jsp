...
<app:layout-member title="Ledgers">
	...
	<table class="table table-striped table-bordered table-hover mt-3">
		...
		<c:forEach var="item" items="${result.contents() }" varStatus="sts">
			<tr>
				...
				<td>${dateTime.formatDateTime(item.modifiedAt())}</td>
				<td>${item.total() }</td>
	...
	<div id="editDialog" class="modal fade">
		<div class="modal-dialog">
			<sf:form method="post" id="editForm" modelAttribute="form" class="modal-content">
				...
				<div class="modal-body">
					<app:form-group label="Type" cssClass="mb-3">
						<sf:select class="form-select" path="type" required="required">
					...
					<div class="form-check">
						<sf:checkbox path="status" class="form-check-input" id="status" /> <label
							for="status" id="statusLabel" class="form-check-label">Active</label>
					</div>
				</div>
